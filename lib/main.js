...
MjPageJob.prototype.run = function() {
    ...
    while (script = scripts[index]) {
        ...
        function html2xhtml(htmlString) {
            ...
            const fullXhtmlString = serializeToString(dom);
            const justMathRegex = /.*(\<math.*\/math\>).*/gs;
            const regexResult = justMathRegex.exec(fullXhtmlString);
            return regexResult[1];
        }

        conf.math = html2xhtml(script.text);
